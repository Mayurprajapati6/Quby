...
export class OwnerNotificationsController {
  ...
  static async getNotifications(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const data = await OwnerNotificationsService.getNotifications(
        req.user!.userId,
  ...
  static async markAllRead(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      await OwnerNotificationsService.markAllRead(req.user!.userId);
  ...
  static async markRead(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      await OwnerNotificationsService.markRead(req.user!.userId, req.params.id);
