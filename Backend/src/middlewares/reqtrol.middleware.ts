...
const TIMEOUT_MS  = 1500;  // ✅ INCREASED FROM 500ms TO 1500ms
...
function fireTrack(payload: Record<string, unknown>): void {
  ...
  axios.post(`${REQTROL_URL}/api/v1/track`, payload, { timeout: 2000 })
...
export async function reqtrolMiddleware(
  ...
  try {
    ...
    // ✅ TRACK EVENT EVEN WHEN REQTROL IS UNREACHABLE
    const responseTimeMs = Date.now() - start;
    fireTrack({
      requestId, userId, endpoint,
      action:         endpoint.split('/').filter(Boolean)[0] ?? 'unknown',
      method, ip, userAgent,
      allowed:        true,  // Failing open = allowed
      reason:         'reqtrol_unreachable',
      limit:          0,
      remaining:      0,
      resetIn:        0,
      service: 'quby', 
